...
@interface MXMaterialAlertWindow ()
/**
...
@implementation MXMaterialAlertWindow
+ (instancetype)defaultWindow {
    static MXMaterialAlertWindow *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[MXMaterialAlertWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    });
    return instance;
}
...
- (void)bringWindowToTop:(NSNotification *)notification {
    if ([notification object] == nil || ![[notification object] isKindOfClass:[MXMaterialAlertWindow class]]) {
        dispatch_async(dispatch_get_main_queue(), ^{
//            [[MXMaterialAlertWindow defaultWindow] setHidden:YES];
            [[MXMaterialAlertWindow defaultWindow] setHidden:NO];
        });
    }
}
...
- (void)handleRotate:(UIInterfaceOrientation)orientation {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        MXMaterialAlert *materialAlert = [[MXMaterialAlertManager defaultManager] currentMaterialAlert];
        if (materialAlert != nil) {
            [[materialAlert materialAlertView] updateView];
        }
    });
}
